...
public extension Segment where Context == SQLiteStatement.CreateContext {
    static func TABLE(_ type: (some Entity).Type, ifNotExists: Bool = true, segments: Segment<Context>...) -> Segment {
        .clause(
            Clause(
                keyword: .table,
                segments: [
...
public extension Segment where Context == SQLiteStatement.CreateContext {
    ...
    static func SCHEMA(_ entity: Entity, ifNotExists: Bool = true, segments: Segment<Context>...) -> Segment {
        ...
        for column in entity.attributes.filter(\.primaryKey) {
            let segment = PRIMARY_KEY(column)
            columnSegments.append(segment)
        }

        for column in entity.attributes.filter({ $0.foreignKey != nil }) {
